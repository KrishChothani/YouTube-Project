...
function EdminDashboard() {
  ...
  //           const res = await axios.get(`http://localhost:8000/api/v1/videos/v/${videoId}`);
  ...
  const handleCheckboxChange = async (videoId) => {
    try {
      ...
      const res = await axios.patch(
        `http://localhost:8000/api/v1/videos/uv/${videoId}/toggle-publish`,
        {},
        { withCredentials: true }
      );
      setLoadData(!load);
  ...
  const deleteVideo = async (videoId) => {
     try {
       ...
       const res = await axios.delete(
         `http://localhost:8000/api/v1/videos/uv/${videoId}`,
         { withCredentials: true }
       );
       setLoadData(!load);
     } catch (error) {
