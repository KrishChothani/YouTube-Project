import React from 'react'

function ChannelSubscribers() {
  return (
    <>
      <div class="flex flex-col gap-y-4 py-4">
        <div class="relative mb-2 rounded-lg bg-white py-2 pl-8 pr-3 text-black">
          <span class="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="2"
              stroke="currentColor"
              aria-hidden="true"
              class="h-5 w-5"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"
              ></path>
            </svg>
          </span>
          <input
            class="w-full bg-transparent outline-none"
            placeholder="Search"
          />
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/3532545/pexels-photo-3532545.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Code Master"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Code Master</h6>
              <p class="text-sm text-gray-300">20K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/3532552/pexels-photo-3532552.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="React Ninja"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">React Ninja</h6>
              <p class="text-sm text-gray-300">40K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/3532549/pexels-photo-3532549.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Async Masters"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Async Masters</h6>
              <p class="text-sm text-gray-300">60K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/2522659/pexels-photo-2522659.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Code Crafters"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Code Crafters</h6>
              <p class="text-sm text-gray-300">80K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/2519823/pexels-photo-2519823.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Tailwind Pro"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Tailwind Pro</h6>
              <p class="text-sm text-gray-300">100K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/2519812/pexels-photo-2519812.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Express Learner"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Express Learner</h6>
              <p class="text-sm text-gray-300">120K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/18264716/pexels-photo-18264716/free-photo-of-man-people-laptop-internet.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Redux Master"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Redux Master</h6>
              <p class="text-sm text-gray-300">140K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1739942/pexels-photo-1739942.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="API Builder"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">API Builder</h6>
              <p class="text-sm text-gray-300">160K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1739856/pexels-photo-1739856.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="React Native Dev"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">React Native Dev</h6>
              <p class="text-sm text-gray-300">180K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1144257/pexels-photo-1144257.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Hook Master"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Hook Master</h6>
              <p class="text-sm text-gray-300">200K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1144261/pexels-photo-1144261.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="CSS Wizard"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">CSS Wizard</h6>
              <p class="text-sm text-gray-300">220K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1144268/pexels-photo-1144268.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Pythonista"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Pythonista</h6>
              <p class="text-sm text-gray-300">240K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1144269/pexels-photo-1144269.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Django Master"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Django Master</h6>
              <p class="text-sm text-gray-300">260K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1144275/pexels-photo-1144275.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="ML Geek"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">ML Geek</h6>
              <p class="text-sm text-gray-300">280K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1144277/pexels-photo-1144277.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="ReactD3"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">ReactD3</h6>
              <p class="text-sm text-gray-300">300K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1144270/pexels-photo-1144270.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Passport Pro"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Passport Pro</h6>
              <p class="text-sm text-gray-300">320K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1144235/pexels-photo-1144235.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Django Rest API"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Django Rest API</h6>
              <p class="text-sm text-gray-300">340K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1144232/pexels-photo-1144232.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="JS Ninja"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">JS Ninja</h6>
              <p class="text-sm text-gray-300">360K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/18264716/pexels-photo-18264716/free-photo-of-man-people-laptop-internet.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Tableau Master"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Tableau Master</h6>
              <p class="text-sm text-gray-300">380K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/18264716/pexels-photo-18264716/free-photo-of-man-people-laptop-internet.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="Socket.IO Expert"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">Socket.IO Expert</h6>
              <p class="text-sm text-gray-300">400K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/18264716/pexels-photo-18264716/free-photo-of-man-people-laptop-internet.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="GraphQL Pro"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">GraphQL Pro</h6>
              <p class="text-sm text-gray-300">420K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/1115822/pexels-photo-1115822.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="MERN Stack"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">MERN Stack</h6>
              <p class="text-sm text-gray-300">440K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/18264716/pexels-photo-18264716/free-photo-of-man-people-laptop-internet.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="CSS Animations"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">CSS Animations</h6>
              <p class="text-sm text-gray-300">460K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/18264716/pexels-photo-18264716/free-photo-of-man-people-laptop-internet.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="ML Image"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">ML Image</h6>
              <p class="text-sm text-gray-300">480K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-white focus:bg-[#ae7aff]">
              <span class="group-focus/btn:hidden">Subscribe</span>
              <span class="hidden group-focus/btn:inline">Subscribed</span>
            </button>
          </div>
        </div>
        <div class="flex w-full justify-between">
          <div class="flex items-center gap-x-2">
            <div class="h-14 w-14 shrink-0">
              <img
                src="https://images.pexels.com/photos/18264716/pexels-photo-18264716/free-photo-of-man-people-laptop-internet.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                alt="React Patterns"
                class="h-full w-full rounded-full"
              />
            </div>
            <div class="block">
              <h6 class="font-semibold">React Patterns</h6>
              <p class="text-sm text-gray-300">500K Subscribers</p>
            </div>
          </div>
          <div class="block">
            <button class="group/btn px-3 py-2 text-black bg-[#ae7aff] focus:bg-white">
              <span class="group-focus/btn:hidden">Subscribed</span>
              <span class="hidden group-focus/btn:inline">Subscribe</span>
            </button>
          </div>
        </div>
      </div>
    </>
  );
}

export default ChannelSubscribers
